...
# ============================================================
# HARDCODED CITY COORDINATES (FALLBACK)
# ============================================================
CITY_COORDS = {
    # Italy
...
def get_coordinates(address):
    ...
    print(f"Nominatim failed, trying to extract city from: {address}")
...
def get_risk_factors(input_data):
    ...
    if input_data['fragile'].iloc[0] == 1: risks.append("Fragile goods")
    if input_data['tracking_gps'].iloc[0] == 0: risks.append("No GPS tracking")
...
def fetch_customs_status(pickup: str, delivery: str, mode: str):
    # This fixes the "N/A" error by providing simulation data if API is missing
...
def trigger_email_alert(context: str, prob: float, threshold: float, to_email: str, extra: dict = None):
    ...
    if res.get("ok"):
        ...
        st.warning(f"Email alert not sent: {res.get('error')}")
...
st.sidebar.markdown("### Shipment Control")
...
def manual_input():
    with st.sidebar.form("manual_entry_form"):
        ...
        valore = st.number_input("Cargo Value (EUR)", min_value=0, value=10000, step=1000)
        peso = st.number_input("Weight (kg)", min_value=0, value=500, step=50)

        st.markdown("#### Route & Transit")
...
if pipeline is None:
    ...
    with tab1:
        ...
        with col2:
            ...
            if 'last_result' in st.session_state:
                ...
                if pred == 1:
                    st.markdown(f"<div class='risk-high'>DELAY PREDICTED</div>", unsafe_allow_html=True)
                else:
    ...
    with tab4:
        ...
        if phone:
            ...
            if driver_weather.get("_error"):
                st.warning(f"Weather API Error: {driver_weather.get('_error')}")
            else:
                ...
                if (rain >= 3) or (snow >= 1) or ((driver_weather.get("wind_mps") or 0) >= 12):
                    st.error("Severe weather detected near driver location.")
                    if st.session_state.get("alert_email_on", False):
                        trigger_email_alert(
    ...
    with tab5:
        ...
        with st.expander("Advanced Configuration", expanded=False):
            ...
            with rc1:
                route_valore = st.number_input("Value (EUR)", 0, value=15000, key="route_value")
        ...
        if st.session_state.route_calculated and 'route_data' in st.session_state:
            ...
            with col_analysis:
                ...
                if rd['pred'] == 1:
                    st.markdown(f"<div class='risk-high'>DELAY</div>", unsafe_allow_html=True)
                else:
                    st.markdown(f"<div class='risk-low'>ON TIME</div>", unsafe_allow_html=True)
    ...
    with tab6:
        ...
        if st.session_state.get("route_calculated") and "route_data" in st.session_state:
            rd = st.session_state.route_data
            context_title = f"Route: {rd['pickup_addr']} to {rd['delivery_addr']}"
        ...
        if active_df is None:
            st.warning("No shipment data loaded. Please calculate a route or update manual input first.")
        else:
            ...
            if new_pred == 1:
                st.markdown(f"<div class='risk-high' style='margin-top:10px;'>SIMULATED STATUS: DELAY</div>",
                            unsafe_allow_html=True)
            else:
            ...
            if user_msg := st.chat_input("Ask about delay reasons, what-if scenarios, mitigation..."):
                ...
                if any(x in msg_lower for x in ["weather", "rain", "storm", "snow"]):
                    ...
                    if w_risk >= 4:
                        reply = f"**Weather Warning:** The weather risk is rated **{w_risk}/5** (Severe). Heavy rain or storms are likely causing delays. I recommend switching to **Rail** or **Air** if possible."
                    elif w_risk >= 3:
                        reply = f"**Weather Alert:** Moderate weather risk ({w_risk}/5). Expect minor slowdowns on the route."
                    else:
                        reply = f"**Weather is Good:** Risk is low ({w_risk}/5). Weather should not impact the delivery time."
                ...
                # 3. Delay Time Logic
                    ...
                    if new_prob > 0.8:
                        reply = f"**Estimated Delay:** High probability of delay (**{new_prob:.1%}**). Historical data suggests a delay of **24-48 hours** for this profile."
                    elif new_prob > 0.5:
                        reply = f"**Estimated Delay:** Moderate risk. You might face a delay of **4-12 hours**."
                    else:
                        reply = "**On Time:** No significant delays are predicted. The shipment should arrive as scheduled."
